...
const app = express();
app.use(cors());
app.use(bodyParser.json()); // Añade esta línea para parsear los cuerpos de las solicitudes JSON

// Middleware de jayson para manejar solicitudes JSON-RPC
app.use('/rpc', server.middleware());

// Endpoints HTTP para probar en Postman
app.post('/createTask', async (req, res) => {
  const params = [req.body];
  methods.createTask(params, (err, result) => {
    if (err) {
...
app.get('/getTask/:id', async (req, res) => {
  ...
  methods.getTask(params, (err, result) => {
    if (err) {
      res.status(500).json(err);
    } else {
      res.json(result);
...
app.put('/updateTask/:id', async (req, res) => {
  ...
  methods.updateTask(params, (err, result) => {
    if (err) {
      res.status(500).json(err);
    } else {
      res.json(result);
...
app.delete('/deleteTask/:id', async (req, res) => {
  const params = [req.params.id];
  methods.deleteTask(params, (err, result) => {
    if (err) {
      res.status(500).json(err);
    } else {
      res.json(result);
...
app.get('/getAllTasks', async (req, res) => {
  methods.getAllTasks([], (err, result) => {
    if (err) {
      res.status(500).json(err);
    } else {
      res.json(result);
